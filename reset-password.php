...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Handle direct access
       ...
       $sql = "SELECT User_ID, Email, reset_token_expiry FROM credentials WHERE reset_token = '$token';";
       ...
       header("Location: http://localhost/athena/reset-password-form.php");
       exit();
}
...
if ($email && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($row['User_ID']) {
        ...
        $updateStmt = $conn->prepare("UPDATE credentials SET reset_token = ?, reset_token_expiry = ? WHERE Email = ?");
        ...
        $resetLink = "http://localhost/athena/reset-password.php?token=$resetToken&email=$email";
