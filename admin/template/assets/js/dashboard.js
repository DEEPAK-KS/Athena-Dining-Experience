...
function setchart(){
  var income = parseInt(document.getElementById('cash_in').value);
  var expense = parseInt(document.getElementById('cash_out').value);  
    var areaData = {
    ...
    var transactionhistoryChartPlugins = {
      beforeDraw: function(chart) {
        ...
        ctx.textBaseline = "middle";
        ctx.fillStyle = "#ffffff";
        var formattedTotal = (income + expense).toLocaleString();
        var text = formattedTotal, 
            textX = Math.round((width - ctx.measureText(text).width) / 2),
            textY = height / 2.4;
